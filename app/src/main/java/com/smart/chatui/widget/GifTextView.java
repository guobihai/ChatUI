...
import com.smart.chatui.util.EmotionUtils;
import com.smart.chatui.util.GifOpenHelper;
...
public class GifTextView extends TextView {
     ...
     * @param text
     */
    public void setSpanText(Handler handler, final String text, boolean isGif,int size) {
        this.handler = handler; // 获得UI的Handler
        this.isGif = isGif;
        spanInfoList = new ArrayList<SpanInfo>();
        if (parseText(text)) {// 对String对象进行解析
            if (parseMessage(this,size)) {
                startPost();
            }
        } else {
            setText(myText);
    ...
    public boolean parseMessage(GifTextView gifTextView,int dbSize) {
        if (gifTextView.myText != null && !gifTextView.myText.equals("")) {
            ...
            for (int i = 0; i < gifTextView.spanInfoList.size(); i++) { // for循环，处理显示多个图片的问题
                 ...
                 */
                int size = ChatUtils.dp2px(gifTextView.getContext(),dbSize);
                if (gifCount != 0) {
    ...
    public static final class TextRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (gifTextView != null) {
                 ...
                 * 。而如果有动图，那么会一直执行
                 */
                if (gifTextView.parseMessage(gifTextView,30)) {
