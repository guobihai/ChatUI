package com.smart.chatui.widget;
...
import com.smart.chatui.util.AudioRecoderUtils;
import com.smart.chatui.util.MsgType;
import com.smart.chatui.util.PopupWindowFactory;
...
public class EmotionInputDetector {
    ...
    private OnScrollToByRecycleView mOnScrollToByRecycleView;

    public EmotionInputDetector setOnScrollToByRecycleView(OnScrollToByRecycleView onScrollToByRecycleView) {
        mOnScrollToByRecycleView = onScrollToByRecycleView;
        return this;
    }
    ...
    public EmotionInputDetector build() {
        ...
        mAudioRecoderUtils.setOnAudioStatusUpdateListener(new AudioRecoderUtils.OnAudioStatusUpdateListener() {
            ...
            public void onUpdate(double db, long time) {
                ...
                mTextView.setText(ChatUtils.long2String(time));
            }

            //录音结束，filePath为保存路径
            @Override
            public void onStop(long time, String filePath) {
                mTextView.setText(ChatUtils.long2String(0));
                MessageInfo messageInfo = new MessageInfo();
                messageInfo.setFilepath(filePath);
                messageInfo.setVoiceTime(time);
                messageInfo.setMsgType(MsgType.SEND_TYPE_VOICE);
                EventBus.getDefault().post(messageInfo);
                String baseUrl = PreferenceUtils.getString(mActivity, "filePath", "");
                LogUtils.sysout("=====baseUrl======="+baseUrl);
                UploadFileToNet.uploadFile(baseUrl, filePath, SystemUitls.initPhoneIMEI(mActivity), MsgType.SEND_TYPE_VOICE);
            }
    ...
    private void unlockContentHeightDelayed() {
        mEditText.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (null != mOnScrollToByRecycleView)
                    mOnScrollToByRecycleView.onScrollToPosition();
            }
        }, 200L);
    }

//    private void recycleViewScrollToPosition() {
//        EasyRecyclerView easyRecyclerView = (EasyRecyclerView) mContentView;
//        ChatAdapter chatAdapter = (ChatAdapter) easyRecyclerView.getRecyclerView().getAdapter();
...
//    }
    ...
    public interface OnScrollToByRecycleView {
